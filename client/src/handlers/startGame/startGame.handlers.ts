...
const startGameSectionReady: HTMLElement = document.querySelector('.start-game__ready');
const startGameSectionBtn: HTMLElement = document.querySelector('.start-game__btn');

const showStartGameSectionReady = (): void => {
    startGameSectionReady.style.display = 'block';
};
  
const hideStartGameSectionReady = (): void => {
    startGameSectionReady.style.display = 'none';
};
...
export const initialiseStartGame = (game: Game, socket: Socket) => {
    ...
    if (isHost) {
        ...
        startGameSectionBtn.addEventListener('click', () => startGame(socket, game));
    } else {
