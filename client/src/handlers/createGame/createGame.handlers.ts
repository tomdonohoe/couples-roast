...
const createGameForm: HTMLElement = document.querySelector('.create-game__form');
const createGameFormInput: HTMLInputElement = document.querySelector('.create-game__name');
...
export const initialiseCreateGame = (game: Game, socket: Socket) => {
    ...
    createGameForm.addEventListener('submit', (event: SubmitEvent) => createGame(event, socket));
    socket.on(GAME_CREATED, (data: GameCreatedData) => onGameCreated(data, game, socket));
};
