...
const roundSectionImage: HTMLImageElement = document.querySelector('.round__image');
...
export const initialiseRoundStarted = (game: Game, socket: Socket) => {
  socket.on(ROUND_STARTED, (data: RoundStartData) => onRoundStarted(data, socket, game));
};
