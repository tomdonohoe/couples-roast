...
const roundResultsSectionImage: HTMLImageElement = document.querySelector('.round-results__image');
const roundResultsSectionCaptions: HTMLElement = document.querySelector('.round-results_captions');
...
const addRoundCaptionsForVoting = (data: Round): void => {
    ...
    for (const caption of captions) {
      ...
      const text = document.createTextNode(`${caption.player.friendlyName}: ${caption.caption}`);
...
export const initialiseStartVoting = (game: Game, socket: Socket) => {
  socket.on(VOTING_START, (data: RoundEndedData) => onStartVoting(data, socket, game));
};
