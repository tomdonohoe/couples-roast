...
const roundSectionForm: HTMLFormElement = document.querySelector('.round__form');
const roundSectionFormInput: HTMLInputElement = document.querySelector('.round__caption');
const roundSectionFormCaptionContainer: HTMLElement = document.querySelector('.round_captionContainer');


const submitRoastCaption = (event: SubmitEvent, socket: Socket, game: Game, data: RoundStartData) => {
...
export const initialisePlayerRoastCaption = (game: Game, data: RoundStartData, socket: Socket) => {
  roundSectionForm.addEventListener('submit', (event: SubmitEvent) =>
    submitRoastCaption(event, socket, game, data),
  );
  socket.on(ROUND_SUBMITTED_PLAYER_CAPTION, (data: RoundPlayerCaption) => onSubmittedRoastCaption(data, game));
};
